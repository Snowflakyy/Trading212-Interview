...
public class TransactionMapper {
    ...
    public TransactionEntity createTransactionRequestToEntity(CreateTransactionRequest request) {
        CryptoWebSocketEntity cryptoWebSocketEntity = cryptoWebSocketRepository.findBySymbol(request.getCryptoSymbol()).orElseThrow(() -> new RuntimeException(("Crypto not found with symbol:" + request.getCryptoSymbol())));
        WalletEntity walletEntity = walletRepository.findByWalletName(request.getWalletName()).orElseThrow(() -> new RuntimeException(("Wallet not found with name:" + request.getWalletName())));

        return TransactionEntity.builder().transactionType(request.getTransactionType()).quantity(request.getQuantity()).pricePurchased(request.getPricePurchased()).cryptoWebSocket(cryptoWebSocketEntity).wallet(walletEntity).build();
    }

    public TransactionDto TransactionEntityToDto(TransactionEntity transaction) {
        return TransactionDto.builder().transactionType(transaction.getTransactionType()).quantity(transaction.getQuantity()).pricePurchased(transaction.getPricePurchased()).crypto(cryptoMapper.cryptoWebSocketEntityToDto(transaction.getCryptoWebSocket())).createdDate(transaction.getCreatedDate()).build();
    }
    ...
    public CreateTransactionResponse TransactionEntityToResponseDto(TransactionEntity transaction) {
        ...
        return CreateTransactionResponse.builder().transactionType(transaction.getTransactionType()).quantity(transaction.getQuantity()).pricePurchased(transaction.getPricePurchased()).crypto(cryptoMapper.cryptoWebSocketToSimpleDto(transaction.getCryptoWebSocket())).createdDate(formattedDate).walletName(walletEntity.getWalletName()).build();
    }

    public List<HoldingDto> mapTransactionToHoldingDto(List<TransactionDto> transactions,Map<String, BigDecimal> currentPrices) {
        ...
        for (Map.Entry<String, List<TransactionDto>> entry : transactionsByCryptoId.entrySet()) {
            ...
            BigDecimal holdingQuantity = calculateHoldingQuantity(transactions);
            ...
            List<TransactionDto> buyTransactions = cryptoTransactions.stream().filter(tx -> tx.getTransactionType() == TransactionType.BUY).sorted(Comparator.comparing(TransactionDto::getCreatedDate).reversed()).toList();

            BigDecimal totalCost = calculateTotalCost(buyTransactions, holdingQuantity);
            ...
            if (totalCost.compareTo(BigDecimal.ZERO) == 0) {
                ...
                profitLossPercentage = profitLoss.divide(totalCost, 4, RoundingMode.HALF_UP).multiply((new BigDecimal("100")));
            }

            HoldingDto holdingDto = HoldingDto.builder().cryptoName((crypto.getName())).cryptoSymbol(crypto.getSymbol()).holdingQuantity(holdingQuantity).holdingReturn(holdingReturn).profitLoss(profitLoss).profitLossPercentage(profitLossPercentage).isProfit(profitLoss.compareTo(BigDecimal.ZERO) >= 0).build();
    ...
    public BigDecimal calculateHoldingQuantity(List<TransactionDto> transactions) {
    ...
    private BigDecimal calculateTotalCost(List<TransactionDto> buyTransactions, BigDecimal remainingTokens) {
        ...
        for (TransactionDto buyTx : buyTransactions) {
            ...
            totalCost = totalCost.add(usedTokens.multiply(buyTx.getPricePurchased()));
